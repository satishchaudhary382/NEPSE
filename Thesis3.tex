...
\hline
\textbf{Variable} & \textbf{Mean} & \textbf{Std. Dev.} & \textbf{Min} & \textbf{Median} & \textbf{Max} & \textbf{Skewness} & \textbf{Kurtosis} & \textbf{JB} & \textbf{JB-p-value} \\
\hline
ln\_total\_tbr & -0.66 & 0.26 & -1.18 & -0.57 & -0.32 & -0.59 & -0.98 & 4.42 & 0.11 \\
ln\_india\_tbr & -0.68 & 0.27 & -1.24 & -0.68 & -0.27 & -0.24 & -1.09 & 2.68 & 0.26 \\
ln\_row\_tbr & -0.64 & 0.23 & -1.12 & -0.59 & -0.23 & -0.14 & -0.96 & 1.89 & 0.39 \\
ln\_reer & 2.03 & 0.07 & 1.92 & 2.04 & 2.16 & 0.10 & -1.42 & 3.86 & 0.14 \\
ln\_nepal\_gdp & 9.92 & 0.43 & 9.29 & 9.78 & 10.63 & 0.32 & -1.36 & 4.21 & 0.12 \\
ln\_india\_gdp & 11.86 & 0.43 & 11.27 & 11.71 & 12.59 & 0.25 & -1.46 & 4.45 & 0.11 \\
ln\_row\_gdp & 14.48 & 0.33 & 13.94 & 14.41 & 14.96 & -0.17 & -1.24 & 3.09 & 0.21 \\
\hline
\multicolumn{10}{l}{\textit{Note. N = 45 for all variables. All variables are in natural logarithms.}} \\
\multicolumn{10}{l}{\textit{JB = Jarque-Bera test statistic for normality.}} \\
\hline
\end{tabular}
\end{table}

Table 1 presents the descriptive statistics for the log-transformed variables utilized in this study, covering the 1980–2024 period (T = 45). The mean and median values for the three trade balance ratios are all negative. This statistically confirms the core premise of this thesis: Nepal has sustained a persistent chronic trade deficit across aggregate, bilateral (India), and floating (ROW) regimes over the past four decades. The standard deviations of these trade balance ratios indicate notable volatility, reflecting the economy's susceptibility to external shocks and shifting domestic absorption capacities over time.

The distributional properties of the variables further justify the econometric approach adopted. The skewness values for the variables deviate from the benchmark of zero, indicating asymmetric distributions. Similarly, the kurtosis values diverge from the normal distribution threshold of 3, suggesting the presence of fat tails—meaning extreme positive or negative shocks (such as the 2015 earthquake or the COVID-19 pandemic) are captured in the data. This is formally confirmed by the Jarque-Bera (JB) statistics and their corresponding p-values, which reject the null hypothesis of normal distribution at conventional significance levels for the majority of the series.

The non-normal, volatile, and asymmetric nature of these macroeconomic time series provides strong empirical justification for two key methodological choices. First, it validates the use of natural logarithms to stabilize variance and interpret coefficients as elasticities. Second, and more importantly, the inherent asymmetry in the data validates the adoption of the Nonlinear ARDL (NARDL) framework. Unlike linear models that assume symmetric, normally distributed adjustments, the NARDL approach is robust to non-normal data and is specifically designed to capture the differential impacts of positive and negative shocks.
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=14cm, height=7cm]{plots/ln_reer.pdf}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=14cm, height=7cm]{plots/ln_nepal_gdp.pdf}
\end{figure}
...
\textbf{Table 4}\\
\textit{Short-Run and Long-Run Adjustment Dynamics}
...
\hline
\textbf{Model} & \multicolumn{2}{c}{\textbf{Short-Run }$\Delta$\textbf{REER}} & \multicolumn{3}{c}{\textbf{Error Correction Term}} \\
\cline{2-3} \cline{4-6}
 & \textbf{Coefficient} & \textbf{Prob.} & \textbf{ECT(-1) Coefficient} & \textbf{t-Statistic} & \textbf{Prob.} \\
\hline
Aggregate & -0.382 & 0.163 & -0.335 & -2.685 & 0.010** \\
India & -0.145 & 0.557 & -0.150 & -1.755 & 0.086* \\
ROW & -1.484 & 0.002*** & -0.839 & -4.572 & 0.000*** \\
\hline
\multicolumn{6}{l}{\textit{Note.} ECT = Error Correction Term. REER = Real Effective Exchange Rate.} \\
\multicolumn{6}{l}{$^{*}p < .10$, $^{**}p < .05$, $^{***}p < .01$.} \\
\hline
\end{tabular}
\end{table}
...
\vspace{0.5cm}
\noindent\textbf{4.5.2 NARDL ROW Dynamic Multipliers}

\begin{figure}[htbp]
    \raggedright
    \textbf{Figure 5}\\
    \textit{NARDL Dynamic Multiplier}\\[0.5em]
    \includegraphics[width=14cm, height=7cm]{plots/dynamic_multipliers_asymmetry.pdf}
\end{figure}

To visually complement the formal long-run Wald test, the NARDL dynamic multipliers are plotted in Figure 5. This graph illustrates the asymmetric adjustment trajectory of Nepal's trade balance with the Rest of the World (ROW) following a unit positive shock (currency appreciation) and a unit negative shock (currency depreciation) over the forecast horizon.

The visual evidence strongly corroborates the statistical findings. The line representing the response to real depreciation (a negative shock) immediately trends downward and settles at a deeply negative equilibrium in the long run. This indicates that currency depreciation unambiguously and persistently worsens the trade balance over time. Conversely, the line representing the response to real appreciation (a positive shock) remains largely flat, hovering close to the zero bound. This suggests that currency appreciation does not yield a corresponding symmetric improvement in the trade balance.

The visual divergence between these two adjustment paths—where depreciation actively harms the trade balance while appreciation fails to generate an equivalent benefit—graphically confirms the presence of long-run asymmetry. Furthermore, the steep, negative trajectory of the depreciation line visually validates the failure of the Marshall-Lerner condition in Nepal's ROW trade; the volume effects of cheaper exports are entirely outweighed by the adverse price effects of more expensive essential imports, creating a structural deterioration rather than a J-curve recovery.
...
\begin{figure}[htbp]
    \raggedright
    \textbf{Figure 6}\\
    \textit{NARDL ROW CUSUM and CUSUM of Squares}\\[0.5em]
    
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{plots/cusum09.pdf}\\[0.5em]
        \small (a) CUSUM
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{plots/cusumq09.pdf}\\[0.5em]
        \small (b) CUSUM of Squares
    \end{minipage}
    
\end{figure}
To conserve space, the CUSUM and CUSUMSQ plots for the ROW NARDL model—the primary model yielding significant long-run co-integration and asymmetry—are presented in Figures 6 (a) and (b) respectively. The statistics remain within the 5\% critical bounds, confirming that the parameters are structurally stable over the sample period despite the exogenous shocks (1990 liberalization, 2015 earthquake, COVID-19). The plots for the Aggregate and India models, which exhibited no co-integration, are omitted from the main text but confirm similar stability during the pre-estimation phase."
...
The empirical evidence strongly favours regime dependency. The India (pegged) model demonstrated no co-integration and symmetric adjustment, confirming that the bilateral peg effectively mutes the transmission of exchange rate shocks to trade flows. Conversely, the ROW (floating) model displayed robust co-integration and significant long-run asymmetry. While the linear ECM estimated a convergence speed of -0.839 for the ROW model, re-estimating the model within the asymmetric NARDL-ECM framework reveals a faster adjustment speed (ECT = -0.926). This indicates that allowing for asymmetry improves the model's speed of correction to long-run equilibrium. Aggregating these two distinct regimes masks the true dynamics of the floating market, proving that exchange rate policy is only relevant for Nepal's trade with the Rest of the World.
...
First, the analysis of the bilateral India trade validates the theoretical premise of the pegged exchange rate regime. Because the NPR is pegged to the INR, real exchange rate fluctuations are largely driven by inflation differentials, which are absorbed by central bank reserve management. Consequently, the NARDL bounds test found no long-run co-integrating relationship, and the Wald test confirmed symmetry. The peg effectively neutralizes the exchange rate as a short-run or long-run corrective tool for the India trade balance. This extends the findings of \textcite{Adhikari2020-rr} by confirming that within a pegged framework, trade flows respond symmetrically (or not at all) to currency valuation changes because the nominal anchor removes the price transmission mechanism.

Second, the analysis of the Rest of the World (ROW) trade reveals where the true asymmetric dynamics lie. When the REER is allowed to decompose into appreciation and depreciation, the long-run Wald test rejects symmetry at the 10\% level. Specifically, the coefficient for real depreciation (-1.800) is highly significant, while the coefficient for appreciation (0.245) is insignificant. This indicates an inherent structural asymmetry: currency depreciation systematically worsens the trade balance, but currency appreciation does not correspondingly improve it. The significantly negative long-run coefficient for real depreciation provides empirical confirmation that the Marshall-Lerner condition does not hold for Nepal's trade with the rest of the world. Because the sum of import and export price elasticities is less than one, the volume effect of depreciation is entirely dominated by the adverse price effect of more expensive essential imports. As \textcite{pathak2020} noted, when demand for essential imports like petroleum, machinery, and pharmaceuticals is price-inelastic, a depreciation simply raises the domestic NPR cost of survival without reducing import volumes. Meanwhile, Nepalese exporters lack the productive capacity to immediately capitalize on the cheaper currency.

Third, the failure of the J-curve to materialize in the long run (Hypothesis 1) confirms the earlier suspicions of \textcite{chaulagai2015}, who identified an "L-Curve" using linear models. While this study employed the advanced NARDL framework to test if non-linearity was hiding the J-curve, the NARDL long-run coefficients for the ROW regime confirm that depreciation has a persistent, negative long-run effect on the trade balance. Therefore, rather than an initial deterioration followed by improvement (J-Curve), or a flat stagnant response (L-Curve as suggested by previous linear studies), Nepal's trade balance with the ROW exhibits a "Worsening Curve"—where depreciation actively deepens the deficit over time due to structural import dependence. The "L-Curve" finding was not just a methodological artifact of linear models; it is a genuine structural feature of Nepal's trade with floating currency regimes. Currency devaluation against the USD and other global currencies acts as an inflationary tax rather than an export stimulus.

Finally, the results strongly justify the theoretical decision to disaggregate the trade data. Had this study relied solely on aggregate data, the F-bounds test would have failed to find co-integration, and the significant asymmetric dynamics operating within the ROW regime would have been masked by the neutralizing effect of the Indian peg. The disaggregated NARDL approach successfully isolates the floating-regime dynamics, proving that exchange rate policy is not a viable standalone tool for addressing Nepal's trade deficit.
...
\end{center}
\begin{center}
\textbf{SUMMARY, CONCLUSION AND RECOMMENDATIONS}
\end{center}
\setstretch{1.5}

\vspace{0.5cm}
\noindent\textbf{5.1 Summary of the Study}

The relationship between exchange rate fluctuations and the trade balance remains one of the most debated topics in international macroeconomics. For decades, Nepal has confronted a chronic and deepening trade deficit that defies traditional corrective mechanisms. According to orthodox economic theory—specifically the Marshall-Lerner condition—currency depreciation should naturally stabilize the trade balance by making exports cheaper and imports more expensive. However, despite the steady depreciation of the Nepalese Rupee (NPR) against convertible currencies over the past two decades, Nepal’s trade deficit has surged. Previous studies attempting to explain this paradox relied predominantly on linear models, which operated under the flawed assumption of symmetry—the notion that an economy responds identically to currency appreciation and depreciation. Consequently, these studies failed to provide a conclusive explanation, often concluding that Nepal’s trade balance suffers from an unresponsive "L-Curve."

This thesis investigated this empirical puzzle by employing the Nonlinear Autoregressive Distributed Lag (NARDL) framework developed by Shin et al. (2014) to test for the existence of an asymmetric J-curve in Nepal. The study utilized annual data spanning 45 years (1980–2024) and uniquely disaggregated Nepal’s trade balance into three distinct regimes: the aggregate trade balance, the bilateral trade balance with India (operating under a fixed peg), and the trade balance with the Rest of the World (ROW, operating under a floating regime). A sequential three-stage estimation process—Linear ARDL bounds testing, Error Correction Model (ECM), and NARDL—was applied to each regime. The core objective was to determine whether the absence of a J-curve in previous literature was a genuine structural feature of the Nepalese economy or merely a methodological artifact of using linear, aggregate models.

\vspace{0.5cm}
\noindent\textbf{5.2 Major Findings}

The empirical analysis yielded three pivotal findings, directly addressing the research hypotheses and resolving the central paradox:

Rejection of the J-Curve Phenomenon (Hypothesis 1): The study found no evidence of a J-curve in any of the three trade regimes. For the aggregate and India models, the F-bounds test revealed no long-run cointegration, rendering the J-curve inapplicable. For the ROW model, where cointegration was robustly established, both the short-run and long-run coefficients for real depreciation were significantly negative. Specifically, the short-run coefficient was -1.484, and the long-run NARDL depreciation coefficient was -1.800. Because depreciation initially worsened the trade balance and continued to deepen the deficit in the long run—rather than showing the expected recovery phase—the J-curve phenomenon definitively does not exist for Nepal.

Presence of Regime-Specific Long-Run Asymmetry (Hypothesis 2): The NARDL Wald test confirmed that asymmetry is not a universal feature but is highly regime-dependent. The aggregate and India models exhibited symmetric adjustment. However, for the ROW model, the null of long-run symmetry was rejected at the 10\% significance level (Wald F = 3.467, p = 0.071). The decomposition revealed a profound structural asymmetry: real depreciation unambiguously worsened the trade balance (-1.800), while real appreciation had no statistically significant effect (0.245, p = 0.774).

Validation of Regime-Dependent Dynamics (Hypothesis 3): The empirical evidence strongly validated the necessity of disaggregating the data by exchange rate regime. The India (pegged) model demonstrated no cointegration and symmetric adjustment, confirming that the bilateral peg effectively mutes the transmission of exchange rate shocks to trade flows. Conversely, the ROW (floating) model displayed robust cointegration, a high speed of adjustment (ECT = -0.926 in the NARDL-ECM), and significant long-run asymmetry. Aggregating these two distinct regimes masked the true asymmetric dynamics of the floating market.

\vspace{0.5cm}
\noindent\textbf{5.3 Conclusions}

The findings of this study provide a profound resolution to Nepal’s trade deficit paradox. The failure of the J-curve in Nepal is not merely an artifact of model misspecification; it is a reflection of deep-seated structural rigidities within the economy, which are only visible when the data is disaggregated.

First, the analysis of the bilateral India trade validates the theoretical premise of the pegged exchange rate regime. Because the NPR is pegged to the INR, real exchange rate fluctuations are largely driven by inflation differentials, which are absorbed by central bank reserve management rather than transmitted to relative prices. The peg effectively neutralizes the exchange rate as a short-run or long-run corrective tool for the India trade balance. Trade flows with India are governed by structural supply-demand factors, agricultural yield cycles, and industrial capacity, not currency valuation.

Second, the analysis of the ROW trade reveals where the true structural failure lies. The significantly negative long-run coefficient for real depreciation provides empirical confirmation that the Marshall-Lerner condition does not hold for Nepal's trade with the rest of the world. Because the sum of import and export price elasticities is less than one, the volume effect of depreciation is entirely dominated by the adverse price effect of more expensive essential imports. Nepal’s reliance on inelastic imports—such as petroleum, pharmaceuticals, and industrial machinery—means that depreciation simply raises the domestic NPR cost of survival without reducing import volumes. Concurrently, Nepalese exporters lack the productive capacity, technological integration, and scale to immediately capitalize on a cheaper currency.

Consequently, rather than an initial deterioration followed by improvement (the traditional J-Curve), or a flat stagnant response (the "L-Curve" suggested by previous linear studies), Nepal's trade balance with the ROW exhibits a "Worsening Curve." Currency devaluation against hard currencies acts as an inflationary tax rather than an export stimulus. The previous "L-Curve" finding was indeed a methodological artifact of linear models that averaged out this severe asymmetric deterioration.

Ultimately, the results prove that relying on aggregate data obscures reality. Exchange rate policy is not a viable standalone tool for addressing Nepal’s trade deficit, and structural import dependence renders traditional macroeconomic adjustments ineffective.

\vspace{0.5cm}
\noindent\textbf{5.4 Policy Recommendations}

Based on the empirical findings, the following multi-tiered policy recommendations are proposed for the Government of Nepal, Nepal Rastra Bank (NRB), and relevant economic ministries:

Abandon Exchange Rate Depreciation as a Deficit-Correction Tool: Policymakers must recognize that deliberately allowing the NPR to depreciate against hard currencies will not resolve the trade deficit. Because import demand is highly inelastic and export capacity is constrained, depreciation will only fuel imported inflation, raise the cost of living, and worsen the current account deficit. The focus must shift from currency adjustment to structural reform.

Prioritize Structural Import Substitution and Export Diversification: Since price competitiveness (via currency depreciation) fails to stimulate exports, Nepal must focus on non-price competitiveness. Policies should incentivize domestic production of import substitutes, particularly in agriculture and basic consumer goods, to reduce the heavy reliance on Indian and global imports. Furthermore, export baskets must be diversified beyond traditional primary commodities into value-added, light manufacturing.

Maintain the India Peg for Macroeconomic Stability: The findings confirm that the peg with India effectively insulates bilateral trade from exchange rate volatility and absorbs exogenous shocks. Given that India accounts for the majority of Nepal's essential imports (including petroleum), maintaining the peg is crucial for macroeconomic stability. However, policymakers should focus on improving the quality and competitiveness of Nepalese goods in the Indian market rather than relying on currency adjustments.

Control Domestic Inflation to Prevent Real Appreciation: The study highlights that post-2010 real appreciation of the NPR against the ROW hurt export competitiveness. Since the nominal rate against the USD floats, preventing real appreciation requires strict control over domestic inflation. NRB must maintain prudent monetary policy to ensure that Nepal’s inflation rate does not significantly exceed that of its global trading partners.

Channel Remittances into Productive Capital: Nepal receives substantial remittance inflows (equivalent to ~25\% of GDP), which finance the import demand that widens the trade deficit. The government must introduce targeted fiscal policies, such as tax incentives and dedicated investment bonds, to channel these remittances away from unproductive sectors (like real estate) and toward productive investments in manufacturing, infrastructure, and export-oriented industries.

\vspace{0.5cm}
\noindent\textbf{5.5 Limitations and Directions for Future Research}

While this study provides robust and novel insights into Nepal’s trade dynamics, certain limitations must be acknowledged, opening avenues for future research:

Data Frequency: Due to the availability of consistent long-term records, this study utilized annual data over a 45-year period. While this provides a robust historical perspective, annual data may smooth over very short-term, quarter-by-quarter dynamics of the J-curve. Future research utilizing quarterly data, if available, could capture these short-run asymmetries more precisely.

Omission of Remittances: Nepal’s import capacity is heavily sustained by remittance inflows. While acknowledged conceptually, remittances were not included as a regressor to preserve the degrees of freedom in the small-sample NARDL framework. Future studies should construct models that integrate remittance cycles with exchange rate asymmetry.

Bilateral Exchange Rate for India: The study employed the multilateral REER for the India model. While the lack of cointegration confirms the peg's insulating effect, constructing a specific bilateral real exchange rate (NPR/INR adjusted for inflation) could provide further granular insights into the specific price transmission mechanism within the pegged regime.

Sectoral Disaggregation: This study disaggregated trade by country regime (India vs. ROW). Future research could further disaggregate the ROW trade by specific commodity groups (e.g., agricultural vs. manufactured goods) to identify which specific sectors drive the "Worsening Curve" phenomenon.
